...
impl EvalMetricChainLength {
    pub fn __repr__(&self) -> String {
        format!(
            "EvalMetricChainLength(average={}, median={}, max={}, min={})",
            self.average, self.median, self.max, self.min
        )
    }
}
...
pub struct EvalMetricGraphProperties {
    /// For each frame, the ratio of nodes that have matched with an existing tracking chain.
    #[pyo3(get)]
    pub match_ratios: Vec<f32>,
    /// For each frame, the ratio of nodes that have conflicts, a conflict occurs
...
impl EvalMetricGraphProperties {
    pub fn __repr__(&self) -> String {
        format!(
            "EvalMetricGraphProperties(match_ratios=[{}], conflict_ratios=[{}])",
            self.match_ratios
                .iter()
                .map(|f| f.to_string())
                .collect::<Vec<String>>()
